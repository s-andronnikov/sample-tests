...
from playwright.async_api import Browser, BrowserContext, BrowserType, Page, ViewportSize
...
class Driver:
    ...
    @classmethod
    async def init_browser(cls, browser: BrowserType) -> None:
        """Initialize browser instance"""
        cls.browser = await browser.launch(
            channel="chrome",
    ...
    @classmethod
    async def new_workspace(cls) -> None:
        """Create a new browser context and page"""
        if cls.browser:
            new_context = await cls.browser.new_context(
    ...
    @classmethod
    async def close_contexts(cls) -> None:
    ...
    @classmethod
    async def goto(cls, url: str) -> None:
        """Navigate to the specified URL"""
        await cls.get_driver().goto(url)

    @classmethod
    async def set_auth_state(cls, auth_state: dict) -> None:
        """Save authentication state"""
        cls.auth_state_path.parent.mkdir(exist_ok=True)
        await cls.get_driver().context.storage_state(path=cls.auth_state_path)
