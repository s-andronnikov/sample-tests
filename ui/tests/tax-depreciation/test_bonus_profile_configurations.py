...
class TestBonusProfileConfigurations:
    ...
    def setup_authenticated_page(self, authenticated_bonus_profile_page: BonusProfilePage) -> BonusProfilePage:
        """Setup authenticated page once for all tests in this class"""
        authenticated_bonus_profile_page.open_with_id(base_settings.depr_case_id)
        assert authenticated_bonus_profile_page.is_page_loaded(), "Bonus profile configurations page failed to load"
        # Store the page instance on the class for all test methods to use
        self.page = authenticated_bonus_profile_page
    ...
    def test_create_bonus_profile(self):
        """Test creating a new bonus profile"""
        # Click the Create button to open the form
        self.page.click_create_button()
        # Verify form is visible
        assert self.page.bonus_profile_dialog.should_be_visible(), "Create form did not appear"

        # Fill the form with random name, Standard calculation method, and random bonus percent
        bonus_profile_name = self.page.fill_bonus_profile_form()
        print(f"Created bonus profile with name: {bonus_profile_name}")
        ...
        # Verify the new bonus profile appears in the grid
        assert self.page.verify_bonus_profile_in_grid(
            bonus_profile_name
        ), f"Bonus profile '{bonus_profile_name}' not found in grid after creation"

    def test_bonus_profile_name_uniqueness_validation(self):
        """Test that the bonus profile creation form validates uniqueness of names"""
        ...
        self.page.bonus_profile_dialog.name_input.fill(existing_name)

        # Click on form to trigger change event
        self.page.bonus_profile_dialog.form_container.click()

        # Verify validation error appears
        # 1. div.input should have class - error
        assert self.page.bonus_profile_dialog.has_validation_error(), "Name input does not have error class"

        # 2. ".labeled-error" should appear with text "Name must be unique."
        error_message_text = self.page.bonus_profile_dialog.get_error_message()
        ...
        assert (
            not self.page.bonus_profile_dialog.is_create_button_enabled()
        ), "Create button should be disabled when validation error exists"

        # Clean up - cancel the form
        self.page.cancel_form()

    def test_edit_bonus_profile(self):
        """Test editing a bonus profile"""
        first_row = self.page.select_first_row()
        first_row.click()
        ...
        self.page.bonus_profile_dialog.should_be_visible()

        # Check that the form title contains the original name
        form_title_text = self.page.bonus_profile_dialog.form_title.get_text()
        ...
        self.page.edit_bonus_profile_name(new_name)

        # Also modify the bonus percent
        new_percent = 45.5
        self.page.edit_bonus_percent(new_percent)
        ...
        assert self.page.verify_bonus_profile_in_grid(new_name), f"Bonus profile '{new_name}' not found in grid after editing"

    def test_delete_bonus_profile(self):
        """Test deleting a bonus profile from the grid and verifying the operation is successful"""
        # Select the first row in the bonus profile grid
        first_row = self.page.select_first_row()
        first_row.click()

        # Get the name of the selected bonus profile
        bonus_profile_name = self.page.get_name_value_by_row(first_row)
        ...
        self.page.verify_delete_confirmation_popup(bonus_profile_name)
        ...
        # Verify the deleted bonus profile no longer appears in the grid
        assert not self.page.verify_bonus_profile_in_grid(
            bonus_profile_name
        ), f"Bonus profile '{bonus_profile_name}' still found in grid after deletion"
